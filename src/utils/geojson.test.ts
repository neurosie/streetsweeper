...
import { joinRoadSegments } from "./geojson";
import { type Road } from "~/server/api/routers/place";

test("JoinRoadSegments", () => {
  const joinedRoads = joinRoadSegments(ROAD_SEGMENTS);

  expect(joinedRoads.length).toBe(1);
  expect(joinedRoads[0]?.properties.lengthMi).toBeCloseTo(1.457);
});

const ROAD_SEGMENTS: Road[] = [
  {
    type: "Feature",
    id: "way/5615586",
    properties: {
      name: "Hoosick Street",
      id: "way/5615586",
      alternateNames: ["hoosick street", "hoosick st"],
      lengthMi: 0.13997545463091232,
    },
    geometry: {
      type: "LineString",
      coordinates: [
